...
type ThreeColors = Record<StatusKey, number> & { bg: number }
...
function getThemeColors() {
    ...
    return {
        ...
        three: {
            primary: hexToInt(primary), secondary: hexToInt(secondary),
            success: hexToInt(success), warning: hexToInt(warning),
            danger: hexToInt(danger),   info: hexToInt(info),
            bg: hexToInt(bg),           accent: hexToInt(accent),
        } as ThreeColors,
    };
}
...
function makeCardTexture(company: string, role: string, statusLabel: string, statusHex: string, isDark: boolean): THREE.CanvasTexture {
    ...
    const pillW = ctx.measureText(labelText).width + 40;
    const pillH = 30, pillX = 18, pillY = 142;
    ctx.fillStyle = statusHex;
...
export function useThreeBackground(mountRef: React.RefObject<HTMLDivElement>) {
    useEffect(() => {
        ...
        JOB_CARDS.forEach((d) => {
            ...
            const mat = new THREE.MeshStandardMaterial({ map: makeCardTexture(d.company, d.role, STATUS_LABELS[statusKey], statusHex, isDark), transparent: true, opacity: 0.82, roughness: 0.15, metalness: 0.4, side: THREE.DoubleSide });
            const mesh = new THREE.Mesh(cardGeo, mat) as unknown as CardMesh;
            ...
            mesh.add(new THREE.LineSegments(edgeGeo, new THREE.LineBasicMaterial({ color: three[statusKey] ?? three.primary, transparent: true, opacity: 0.55 })));
            scene.add(mesh);
        ...
        const buildLines = () => {
            ...
            for (let i = 0; i < cardMeshes.length; i++) {
                for (let j = i + 1; j < cardMeshes.length; j++) {
                    ...
                    lineGroup.add(new THREE.Line(geo, new THREE.LineBasicMaterial({ color: three[cardMeshes[i].userData.statusKey] ?? three.primary, transparent: true, opacity: (1 - dist / 22) * 0.25 })));
                }
            }
        };
        buildLines();
        ...
        columnColors.forEach((key, i) => {
            ...
            const plane = new THREE.Mesh(
                new THREE.PlaneGeometry(7, 28),
                new THREE.MeshBasicMaterial({ color: three[key], transparent: true, opacity: 0.03, side: THREE.DoubleSide })
            );
            plane.position.set(x, 0, -18);
            scene.add(plane);
            scene.add(new THREE.Line(
                new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(x - 3.5, 14, -17.8), new THREE.Vector3(x + 3.5, 14, -17.8)]),
                new THREE.LineBasicMaterial({ color: three[key], transparent: true, opacity: 0.4 })
            ));
        });
        ...
        for (let i = 0; i < 30; i++) {
            ...
            const dot = new THREE.Mesh(new THREE.SphereGeometry(0.1 + Math.random() * 0.12, 10, 10), new THREE.MeshStandardMaterial({ color: three[key], emissive: three[key], emissiveIntensity: 1.2, roughness: 0 }));
            dot.position.set((Math.random() - 0.5) * 52, (Math.random() - 0.5) * 36, (Math.random() - 0.5) * 18);
            (dot as any).userData = { vy: (Math.random() - 0.5) * 0.008, vx: (Math.random() - 0.5) * 0.006, pulse: Math.random() * Math.PI * 2 };
            dots.push(dot); scene.add(dot);
        }

        const arc  = new THREE.Mesh(new THREE.TorusGeometry(16, 0.035, 8, 100, Math.PI * 1.4), new THREE.MeshBasicMaterial({ color: three.success, transparent: true, opacity: 0.15 }));
        arc.rotation.z = -Math.PI / 2; arc.position.z = -14; scene.add(arc);
        const arc2 = new THREE.Mesh(new THREE.TorusGeometry(22, 0.025, 8, 120, Math.PI * 0.8), new THREE.MeshBasicMaterial({ color: three.info, transparent: true, opacity: 0.1 }));
        arc2.rotation.z = Math.PI * 0.6; arc2.position.z = -18; scene.add(arc2);

        const applyTheme = () => {
            const t = getThemeColors(); css = t.css; three = t.three;
            keyLight.color.setHex(three.info); fillLight.color.setHex(three.secondary);
            (scene.fog as THREE.FogExp2).color.setHex(three.bg);
            const nowDark = document.documentElement.classList.contains("dark");
            cardMeshes.forEach((card, i) => {
                const d = JOB_CARDS[i], statusKey = d.status as StatusKey;
                const mat = card.material as THREE.MeshStandardMaterial;
                if (mat.map) mat.map.dispose();
                mat.map = makeCardTexture(d.company, d.role, STATUS_LABELS[statusKey], t.css[statusKey] ?? "#6366f1", nowDark);
                mat.map.needsUpdate = true; mat.needsUpdate = true;
                (card.children[0] as THREE.LineSegments).material = new THREE.LineBasicMaterial({ color: t.three[statusKey] ?? t.three.primary });
            });
        };
