...
const APP_CARDS = [
    ...
    { company: "MIT",       role: "MS Comp Sci",     status: "warning",   x:  -9, y: -9,  z:  1  },
    { company: "Stanford",  role: "MS CS / AI",      status: "secondary", x:  16, y:  8,  z: -7  },
    { company: "Notion",    role: "Full Stack",      status: "danger",    x:   3, y: -13, z:  3  },
    { company: "GitHub",    role: "Infra Eng",       status: "success",   x: -17, y: -1,  z:  0  },
    { company: "Harvard",   role: "MBA",             status: "info",      x:  10, y:  13, z: -9  },
    { company: "CMU",       role: "MSML Program",    status: "warning",   x:  -4, y:  10, z:  2  },
    { company: "Google",    role: "SWE L4",          status: "success",   x: -20, y:  9,  z: -4  },
    { company: "Berkeley",  role: "MEng EECS",       status: "info",      x:  20, y:  0,  z: -5  },
    { company: "Columbia",  role: "MS Data Science", status: "secondary", x:  -7, y: -15, z: -2  },
    { company: "Shopify",   role: "Senior SWE",      status: "danger",    x:  18, y: -9,  z: -1  },
] as const;
...
function getThemeColors() {
    ...
    const primary = hslToHex(get("--heroui-primary")), secondary = hslToHex(get("--heroui-secondary"));
    const success = hslToHex(get("--heroui-success")),  warning   = hslToHex(get("--heroui-warning"));
    const danger  = hslToHex(get("--heroui-danger")),   info      = hslToHex(get("--heroui-info"));
    const bg      = hslToHex(get("--heroui-background")),accent   = hslToHex(get("--heroui-accent"));
    return {
        css: { primary, secondary, success, warning, danger, info, bg, accent },
        three: { primary: hexToInt(primary), secondary: hexToInt(secondary), success: hexToInt(success), warning: hexToInt(warning), danger: hexToInt(danger), info: hexToInt(info), bg: hexToInt(bg), accent: hexToInt(accent) },
    };
}
...
function makeCardTexture(company: string, role: string, statusLabel: string, statusHex: string, isDark: boolean): THREE.CanvasTexture {
    const W = 420, H = 230, canvas = document.createElement("canvas");
    canvas.width = W; canvas.height = H;
    const ctx = canvas.getContext("2d")!;
    ctx.fillStyle = isDark ? "#0f172a" : "#f8fafc"; roundRect(ctx, 0, 0, W, H, 14); ctx.fill();
    ctx.fillStyle = statusHex; roundRect(ctx, 0, 0, W, 72, { tl: 14, tr: 14, bl: 0, br: 0 }); ctx.fill();
    ctx.fillStyle = "rgba(0,0,0,0.18)"; roundRect(ctx, 0, 0, W, 72, { tl: 14, tr: 14, bl: 0, br: 0 }); ctx.fill();
    ctx.fillStyle = "#fff"; ctx.font = "bold 34px system-ui, sans-serif"; ctx.fillText(company, 20, 48);
    ctx.fillStyle = isDark ? "#e2e8f0" : "#1e293b"; ctx.font = "bold 22px system-ui, sans-serif"; ctx.fillText(role, 20, 108);
    ctx.strokeStyle = isDark ? "#1e293b" : "#e2e8f0"; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(20, 128); ctx.lineTo(W - 20, 128); ctx.stroke();
    ctx.font = "bold 16px monospace";
    const pillW = ctx.measureText(statusLabel.toUpperCase()).width + 40, pillH = 30, pillX = 18, pillY = 142;
    ctx.fillStyle = statusHex; roundRect(ctx, pillX, pillY, pillW, pillH, 6); ctx.fill();
    ctx.fillStyle = "rgba(0,0,0,0.20)"; roundRect(ctx, pillX, pillY, pillW, pillH, 6); ctx.fill();
    ctx.fillStyle = "#fff"; ctx.fillText(statusLabel.toUpperCase(), pillX + 20, pillY + pillH / 2 + 6);
    ctx.strokeStyle = isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.10)"; ctx.lineWidth = 1;
...
function useThreeBackground(mountRef: React.RefObject<HTMLDivElement>) {
    useEffect(() => {
        ...
        const scene = new THREE.Scene(), camera = new THREE.PerspectiveCamera(52, width / height, 0.1, 300);
        camera.position.set(0, 0, 32); scene.fog = new THREE.FogExp2(three.bg, 0.022);
        scene.add(new THREE.AmbientLight(0xffffff, 0.4));
        const keyLight = new THREE.PointLight(three.info, 4, 70); keyLight.position.set(-16, 14, 14); scene.add(keyLight);
        const fillLight = new THREE.PointLight(three.secondary, 3, 70); fillLight.position.set(16, -10, 10); scene.add(fillLight);
        interface CM extends THREE.Mesh { userData: { vx: number; vy: number; vz: number; rx: number; ry: number; statusKey: StatusKey } }
        const cardMeshes: CM[] = [], cardGeo = new THREE.PlaneGeometry(5.8, 3.3);
        APP_CARDS.forEach((d) => {
            const sk = d.status as StatusKey, isDark = document.documentElement.classList.contains("dark");
            const mat = new THREE.MeshStandardMaterial({ map: makeCardTexture(d.company, d.role, STATUS_LABELS[sk], css[sk] ?? "#6366f1", isDark), transparent: true, opacity: 0.82, roughness: 0.15, metalness: 0.4, side: THREE.DoubleSide });
            const mesh = new THREE.Mesh(cardGeo, mat) as CM;
            mesh.position.set(d.x, d.y, d.z); mesh.rotation.set((Math.random() - 0.5) * 0.3, (Math.random() - 0.5) * 0.5, (Math.random() - 0.5) * 0.15);
            mesh.userData = { vx: (Math.random() - 0.5) * 0.005, vy: (Math.random() - 0.5) * 0.004, vz: (Math.random() - 0.5) * 0.003, rx: (Math.random() - 0.5) * 0.0008, ry: (Math.random() - 0.5) * 0.001, statusKey: sk };
            mesh.add(new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(5.8, 3.3, 0.01)), new THREE.LineBasicMaterial({ color: three[sk] ?? three.primary, transparent: true, opacity: 0.55 })));
            scene.add(mesh); cardMeshes.push(mesh);
        });
        const lineGroup = new THREE.Group(); scene.add(lineGroup);
        const buildLines = () => { lineGroup.clear(); for (let i = 0; i < cardMeshes.length; i++) for (let j = i + 1; j < cardMeshes.length; j++) { const dist = cardMeshes[i].position.distanceTo(cardMeshes[j].position); if (dist > 22) continue; lineGroup.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints([cardMeshes[i].position.clone(), cardMeshes[j].position.clone()]), new THREE.LineBasicMaterial({ color: three[cardMeshes[i].userData.statusKey] ?? three.primary, transparent: true, opacity: (1 - dist / 22) * 0.25 }))); } };
        buildLines();
        const dots: THREE.Mesh[] = [], dotKeys: StatusKey[] = ["success", "info", "warning", "danger", "secondary"];
        for (let i = 0; i < 30; i++) { const key = dotKeys[i % dotKeys.length], dot = new THREE.Mesh(new THREE.SphereGeometry(0.1 + Math.random() * 0.12, 10, 10), new THREE.MeshStandardMaterial({ color: three[key], emissive: three[key], emissiveIntensity: 1.2, roughness: 0 })); dot.position.set((Math.random() - 0.5) * 52, (Math.random() - 0.5) * 36, (Math.random() - 0.5) * 18); (dot as any).userData = { vy: (Math.random() - 0.5) * 0.008, vx: (Math.random() - 0.5) * 0.006, pulse: Math.random() * Math.PI * 2 }; dots.push(dot); scene.add(dot); }
        const arc = new THREE.Mesh(new THREE.TorusGeometry(16, 0.035, 8, 100, Math.PI * 1.4), new THREE.MeshBasicMaterial({ color: three.success, transparent: true, opacity: 0.15 })); arc.rotation.z = -Math.PI / 2; arc.position.z = -14; scene.add(arc);
        const arc2 = new THREE.Mesh(new THREE.TorusGeometry(22, 0.025, 8, 120, Math.PI * 0.8), new THREE.MeshBasicMaterial({ color: three.info, transparent: true, opacity: 0.1 })); arc2.rotation.z = Math.PI * 0.6; arc2.position.z = -18; scene.add(arc2);
        const observer = new MutationObserver(() => { const t = getThemeColors(); css = t.css; three = t.three; const nowDark = document.documentElement.classList.contains("dark"); cardMeshes.forEach((card, i) => { const d = APP_CARDS[i], sk = d.status as StatusKey, mat = card.material as THREE.MeshStandardMaterial; if (mat.map) mat.map.dispose(); mat.map = makeCardTexture(d.company, d.role, STATUS_LABELS[sk], t.css[sk] ?? "#6366f1", nowDark); mat.map.needsUpdate = true; mat.needsUpdate = true; }); });
        ...
        window.addEventListener("mousemove", onMM); window.addEventListener("resize", onResize);
        let frameId: number; const clock = new THREE.Clock(); let lineTimer = 0;
        const animate = () => { frameId = requestAnimationFrame(animate); const t = clock.getElapsedTime(); camera.position.x += (mouse.x * 3.5 - camera.position.x) * 0.03; camera.position.y += (mouse.y * 2 - camera.position.y) * 0.03; camera.lookAt(scene.position); cardMeshes.forEach((c, i) => { const d = c.userData; c.position.x += d.vx; c.position.y += d.vy + Math.sin(t * 0.4 + i) * 0.002; c.position.z += d.vz; c.rotation.x += d.rx; c.rotation.y += d.ry; if (Math.abs(c.position.x) > 28) d.vx *= -1; if (Math.abs(c.position.y) > 20) d.vy *= -1; if (Math.abs(c.position.z) > 14) d.vz *= -1; }); lineTimer += 0.016; if (lineTimer > 0.5) { buildLines(); lineTimer = 0; } dots.forEach((dot) => { const d = (dot as any).userData; dot.position.x += d.vx; dot.position.y += d.vy; if (Math.abs(dot.position.x) > 28) d.vx *= -1; if (Math.abs(dot.position.y) > 20) d.vy *= -1; (dot.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.7 + 0.6 * Math.sin(t * 2.5 + d.pulse); }); arc.rotation.z = -Math.PI / 2 + t * 0.06; arc2.rotation.z = Math.PI * 0.6 - t * 0.035; keyLight.position.x = Math.sin(t * 0.38) * 22; keyLight.position.y = Math.cos(t * 0.28) * 14; fillLight.position.x = Math.cos(t * 0.33) * 20; fillLight.position.y = Math.sin(t * 0.25) * 12; renderer.render(scene, camera); };
        animate();
        return () => { cancelAnimationFrame(frameId); observer.disconnect(); window.removeEventListener("mousemove", onMM); window.removeEventListener("resize", onResize); renderer.dispose(); if (mount.contains(renderer.domElement)) mount.removeChild(renderer.domElement); };
    }, []);
}
